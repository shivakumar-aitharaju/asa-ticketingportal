...
class AnalyticsService {
    ...
    async getDashboardOverview(departmentId) {
    ...
    `;
        const [total, open, inProgress, escalated, resolved, closed, slaBreached] = await Promise.all([
            conn.query(base()),
            conn.query(base(`AND t.status = '${ticket_status_enum_1.TicketStatus.Open}'`)),
            conn.query(base(`AND t.status = '${ticket_status_enum_1.TicketStatus.InProgress}'`)),
            conn.query(base(`AND t.is_escalated = true AND t.status NOT IN ('resolved','closed')`)),
            conn.query(base(`AND t.status = '${ticket_status_enum_1.TicketStatus.Resolved}'`)),
            conn.query(base(`AND t.status = '${ticket_status_enum_1.TicketStatus.Closed}'`)),
            conn.query(`
    ...
    async getTicketTrend(days = 30, departmentId) {
    ...
    async getAgentPerformance(departmentId) {
    ...
    async getStatusDistribution(departmentId) {
        const result = await this.dataSource.query(`
      SELECT
        t.status,
        COUNT(*) AS count
    ...
    async getPriorityDistribution(departmentId) {
        const result = await this.dataSource.query(`
      SELECT
        t.priority,
        COUNT(*) AS count
    ...
    async getMonthlyTrend(year, departmentId) {
        const result = await this.dataSource.query(`
      SELECT
    ...
    async getAvgResolutionTime(departmentId) {
        const result = await this.dataSource.query(`
      SELECT
    ...
    async getSLATrend(days, departmentId) {
        const result = await this.dataSource.query(`
      SELECT
