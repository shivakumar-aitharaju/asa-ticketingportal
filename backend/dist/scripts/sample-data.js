...
function randomDateInMonth(year, month, dayIndex, totalDays, rand) {
...
const MANAGERS = [
    { firstName: 'Shivakumar', lastName: 'Aitharaju', email: 'shivakumar.aitharaju@asaind.co.in', dept: 'Tech Queries', role: user_role_enum_1.UserRole.Manager },
    { firstName: 'Nitesh', lastName: 'Gupta', email: 'nitesh.gupta@asaind.co.in', dept: 'Accounting', role: user_role_enum_1.UserRole.Manager },
    { firstName: 'Deepak', lastName: 'Khatri', email: 'deepak.khatri@asaind.co.in', dept: 'Billing', role: user_role_enum_1.UserRole.Manager },
    { firstName: 'Manish', lastName: 'Karande', email: 'manish.karande@asaind.co.in', dept: 'Agency', role: user_role_enum_1.UserRole.Manager },
    { firstName: 'Harsh', lastName: 'Agrawal', email: 'harsh.agrawal@asaind.co.in', dept: null, role: user_role_enum_1.UserRole.Manager },
    { firstName: 'Kriyesh', lastName: 'Shetty', email: 'kriyesh.shetty@asaind.co.in', dept: null, role: user_role_enum_1.UserRole.Manager },
];
const AGENTS = [
...
const STATUS_POOLS = [
    [...Array(70).fill(ticket_status_enum_1.TicketStatus.Closed), ...Array(25).fill(ticket_status_enum_1.TicketStatus.Resolved), ...Array(5).fill(ticket_status_enum_1.TicketStatus.PendingClient)],
    [...Array(65).fill(ticket_status_enum_1.TicketStatus.Closed), ...Array(25).fill(ticket_status_enum_1.TicketStatus.Resolved), ...Array(10).fill(ticket_status_enum_1.TicketStatus.PendingClient)],
    [...Array(50).fill(ticket_status_enum_1.TicketStatus.Closed), ...Array(25).fill(ticket_status_enum_1.TicketStatus.Resolved), ...Array(15).fill(ticket_status_enum_1.TicketStatus.InProgress), ...Array(10).fill(ticket_status_enum_1.TicketStatus.Assigned)],
    [...Array(35).fill(ticket_status_enum_1.TicketStatus.Closed), ...Array(25).fill(ticket_status_enum_1.TicketStatus.Resolved), ...Array(25).fill(ticket_status_enum_1.TicketStatus.InProgress), ...Array(15).fill(ticket_status_enum_1.TicketStatus.Assigned)],
    [...Array(10).fill(ticket_status_enum_1.TicketStatus.Closed), ...Array(15).fill(ticket_status_enum_1.TicketStatus.Resolved), ...Array(30).fill(ticket_status_enum_1.TicketStatus.InProgress), ...Array(25).fill(ticket_status_enum_1.TicketStatus.Assigned), ...Array(20).fill(ticket_status_enum_1.TicketStatus.Open)],
];
const PRIORITY_POOL = [
    ...Array(30).fill(priority_enum_1.Priority.Low),
    ...Array(50).fill(priority_enum_1.Priority.Medium),
    ...Array(15).fill(priority_enum_1.Priority.High),
    ...Array(5).fill(priority_enum_1.Priority.Critical),
];
const MONTHLY_DIST = [
...
async function sampleData() {
    ...
    const messageRepo = data_source_1.AppDataSource.getRepository(ticket_message_entity_1.TicketMessage);
    const historyRepo = data_source_1.AppDataSource.getRepository(ticket_status_history_entity_1.TicketStatusHistory);
    const assignmentRepo = data_source_1.AppDataSource.getRepository(ticket_assignment_entity_1.TicketAssignment);
    const slaRepo = data_source_1.AppDataSource.getRepository(sla_configuration_entity_1.SLAConfiguration);
    const slaTrackRepo = data_source_1.AppDataSource.getRepository(sla_tracking_entity_1.SLATracking);
    ...
    const slaByPriority = {
        [priority_enum_1.Priority.Critical]: slaConfigs.find(s => s.priority === priority_enum_1.Priority.Critical),
        [priority_enum_1.Priority.High]: slaConfigs.find(s => s.priority === priority_enum_1.Priority.High),
        [priority_enum_1.Priority.Medium]: slaConfigs.find(s => s.priority === priority_enum_1.Priority.Medium),
        [priority_enum_1.Priority.Low]: slaConfigs.find(s => s.priority === priority_enum_1.Priority.Low),
    };
    ...
    for (let mIdx = 0; mIdx < MONTHS.length; mIdx++) {
        ...
        for (const { dept: deptName, count } of monthDist) {
            ...
            for (let t = 0; t < count; t++) {
                ...
                const priority = pick(PRIORITY_POOL, rand);
                ...
                const isResolved = [ticket_status_enum_1.TicketStatus.Resolved, ticket_status_enum_1.TicketStatus.Closed].includes(status);
                const isAssigned = status !== ticket_status_enum_1.TicketStatus.Open;
                ...
                const resolvedAt = isResolved
                    ...
                    : null;
                const closedAt = status === ticket_status_enum_1.TicketStatus.Closed && resolvedAt
                ...
                await data_source_1.AppDataSource.query(`UPDATE tickets SET created_at = $1, updated_at = $2 WHERE id = $3`, [createdAt, closedAt ?? resolvedAt ?? createdAt, ticket.id]);
                const histories = [
                    { ticketId: ticket.id, fromStatus: undefined, toStatus: ticket_status_enum_1.TicketStatus.Open, changedById: client.id }
                ];
                if (isAssigned && assignedAgent) {
                    histories.push({ ticketId: ticket.id, fromStatus: ticket_status_enum_1.TicketStatus.Open, toStatus: ticket_status_enum_1.TicketStatus.Assigned, changedById: assignedAgent.id });
                }
                if ([ticket_status_enum_1.TicketStatus.InProgress, ticket_status_enum_1.TicketStatus.Resolved, ticket_status_enum_1.TicketStatus.Closed].includes(status) && assignedAgent) {
                    histories.push({ ticketId: ticket.id, fromStatus: ticket_status_enum_1.TicketStatus.Assigned, toStatus: ticket_status_enum_1.TicketStatus.InProgress, changedById: assignedAgent.id });
                }
                if (isResolved && assignedAgent) {
                    histories.push({ ticketId: ticket.id, fromStatus: ticket_status_enum_1.TicketStatus.InProgress, toStatus: ticket_status_enum_1.TicketStatus.Resolved, changedById: assignedAgent.id });
                }
                if (status === ticket_status_enum_1.TicketStatus.Closed && assignedAgent) {
                    histories.push({ ticketId: ticket.id, fromStatus: ticket_status_enum_1.TicketStatus.Resolved, toStatus: ticket_status_enum_1.TicketStatus.Closed, changedById: assignedAgent.id });
                }
                for (const h of histories) {
                    await historyRepo.save(historyRepo.create(h));
                }
                if (isAssigned && assignedAgent) {
                    const systemUser = await userRepo.findOne({ where: { role: user_role_enum_1.UserRole.Admin } });
                ...
                await data_source_1.AppDataSource.query(`INSERT INTO ticket_messages (ticket_id, author_id, content, is_client_facing, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$5)`, [ticket.id, client.id, description, true, createdAt]);
                if (isAssigned && assignedAgent && firstResponseAt) {
                    const ackContent = pick(AGENT_RESPONSES, rand);
                    await data_source_1.AppDataSource.query(`INSERT INTO ticket_messages (ticket_id, author_id, content, is_client_facing, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$5)`, [ticket.id, assignedAgent.id, ackContent, true, firstResponseAt]);
                }
                if (isResolved && assignedAgent && resolvedAt) {
                    const resContent = pick(RESOLUTION_MESSAGES, rand);
                    await data_source_1.AppDataSource.query(`INSERT INTO ticket_messages (ticket_id, author_id, content, is_client_facing, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$5)`, [ticket.id, assignedAgent.id, resContent, true, resolvedAt]);
                }
                ...
                if (slaConfig) {
                    ...
                    let slaStatus;
                    if (isResolved) {
                        slaStatus = resolvedAt && resolvedAt <= resDue ? sla_tracking_entity_1.SLAStatus.Met : sla_tracking_entity_1.SLAStatus.Breached;
                    }
                    else {
                        const now = new Date();
                        slaStatus = now > resDue ? sla_tracking_entity_1.SLAStatus.Breached : now > addMinutes(resDue, -60) ? sla_tracking_entity_1.SLAStatus.AtRisk : sla_tracking_entity_1.SLAStatus.OnTrack;
                    }
